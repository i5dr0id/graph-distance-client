    ...
    <v-card color="h-50 ma-0">
      <div class="fit" ref="paperBox">
        <div v-show="showSourceGraphLayout" class="fit sourceGraph drop-loader">
          <div id="sideRpaper" class="h-90 fit w-100 paperBox"></div>
        </div>
        <div v-show="!showSourceGraphLayout" class="fit drop-loader">
          <loading class="drop-loader-spinner" :active.sync="isLoadingSource" :can-cancel="true"
            :on-cancel="onCancel" :is-full-page="fullPage"></loading>
          <vue-dropzone v-show="!isLoadingSource" ref="myVueDropzone" @vdropzone-file-added="vfileAddedSource"
            @vdropzone-complete="afterCompleteSource" id="dropzoneSource" :options="dropzoneOptions"
            :useCustomSlot="true" class="fit"></vue-dropzone>
        </div>
      </div>
    </v-card>
  </v-card>
...
export default {
  ...
  methods: {
    ...mapMutations(['mutateMatchGraph', 'mutateMatchNode']),
    ...
    afterCompleteSource(file) {
      ...
      graph.data.forEach((element, index) => {
        ...
        element.splice(1).forEach((ev, idx) => {
          console.log('NODE: ', element[0], 'EDGE: ', ev);
          graphSource.addEdge(element[0], ev);
        });
      });
      console.log('sourceObj: ', this.sourceObj);
      this.axios.post('https://graph-distance-api.herokuapp.com/graph-distance/api/v1.0/graph', { sourceobj: this.sourceObj, nodes: this.nodes }).then(
        (res) => {
          ...
          this.mutateMatchGraph(this.sourceObj);
          this.mutateMatchNode(this.nodes);
          this.$swal({
            toast: true,
            position: 'top-end',
            ...
            text: 'Matching Graph loaded!',
          });
        },
      );
      this.doAjax();
      const layout = new Layout(graphSource);
      const renderer = new Renderer('#sideRpaper', graphSource, this.$refs.paperBox.clientWidth, this.$refs
        .paperBox.clientHeight);
      renderer.draw();
      this.redrawSource = function () {
        layout.layout();
        renderer.draw();
      };
      // this.btnRedrawSource(redrawSource);
    },
